...
export function AboutSustainability({ data }: AboutSustainabilityProps) {
  ...
  const title = data?.sustainabilityTitle || "Doğayla Uyumlu, Geleceğe Saygılı.";
  const description = data?.sustainabilityDescription || "Bugünün binalarını inşa ederken yarının kaynaklarını tüketmiyoruz. Gezgin İnşaat, ekolojik ayak izini minimize eden, yenilenebilir enerji entegrasyonlu ve çevre dostu malzeme odaklı projeleriyle yarınlara nefes aldırıyor.";
    ...
    <section className="relative py-24 md:py-32 min-h-[50vh] flex items-center overflow-hidden">
      ...
      <div className="relative z-10 max-w-[1920px] mx-auto px-4 md:px-12">
        <div className="max-w-2xl text-white">
          <FadeIn direction="up">
            <h2 className="text-primary-container font-label uppercase tracking-[0.4em] mb-8 text-sm">{preTitle}</h2>
            <h3 className="text-4xl md:text-6xl font-headline font-black tracking-tighter mb-10 leading-none">{title}</h3>
            <p className="text-lg md:text-xl text-stone-300 font-light leading-relaxed mb-12">
              {description}
            </p>
            <Link href="/blog" className="inline-flex items-center gap-4 text-primary-container font-bold tracking-widest group hover:text-primary transition-colors">
                 RAPORUMUZU İNCELEYİN
